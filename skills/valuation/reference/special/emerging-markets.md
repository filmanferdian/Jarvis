...
- Operationalize the probability-weighted approach with the `stress.py` Monte Carlo: a single cycle factor co-moves the macro-sensitive drivers (rate, margin, credit, growth) so country and cycle risk land in the cash flows, not the discount rate. Keep the cost of capital at the fair country level (do not also add a country premium) to avoid double-counting; report P10/P50/P90 and the probability the value exceeds the price. This is the many-scenario generalization of the book's two-scenario method.
