...
- **Minority interests** — third-party owners' claims on consolidated subsidiaries. Use **proportional market value** if listed; otherwise a separate valuation via **DCF, multiples, or a tracking portfolio** for the subsidiary.
  - **Consistency rule (modern; critical for holding companies / part-owned subsidiaries).** The enterprise DCF values **100% of the consolidated operations**, so the minority must be subtracted on the **same valuation basis** as that EV. Two coherent methods, never mixed: **(a) consolidated DCF** — value the whole at one blended multiple and subtract the minority's share at that *same* multiple (≈ minority's share of consolidated EBITDA × the DCF-implied EV/EBITDA); or **(b) full sum-of-the-parts** — value each subsidiary at its own multiple and take the parent's ownership of each (see `special/sum-of-parts.md`). Do **not** subtract a premium/SOTP-valued minority from a blended consolidated EV: that double-penalizes the parent and understates equity. Conversely, **book value of minority interest usually understates** a stake in a high-ROIC subsidiary, so "subtract minority at book" flatters the parent — surface **book vs. fair value** as an explicit checkpoint choice when the minority is large. *(Telkom Indonesia: SingTel's 30.1% of Telkomsel was the single biggest swing in the bridge — ~Rp85tn at fair value vs ~Rp22tn at book, flipping the verdict.)*
