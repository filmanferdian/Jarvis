...
**Well:**
- Checked every worktree for unmerged commits and uncommitted files before deleting any. Two of the nine held real uncommitted work: an OpenHuman architecture-eval backlog entry written 2026-06-16 and never committed, and 208KB of untracked job-scanner scripts. A blind `worktree remove --force` sweep would have destroyed both silently. The backlog entry is now on main; everything else is archived outside the repo with a README explaining provenance.
- Caught a wrong justification in my own code comment before it shipped. The first version claimed `\b` would let `ign` match inside "foreign.com", which is false. The real difference is that `\b` counts underscore as a word character. A comment that sounds authoritative and is wrong is worse than no comment, because the next person trusts it.
- Committed the verification script this time instead of rebuilding it. The v3.38.2 retrospective said to; doing it immediately rather than deferring means the next weekly review inherits a 26-case self-test rather than someone's fresh guess.
- The matcher was verified against the 185 real outlet names from the week, not just synthetic cases, and the collision cases it exists to prevent ("Foreign Policy", "The Dispatch", "Komodo") are permanent assertions in the script rather than one-off checks.

**Wrong:**
- Two sessions shipped different work under the same version number. The later ship merged on top of v3.39.1 without bumping `package.json`, so production reported a version that did not identify what was running. Renumbered it to v3.39.2 here. The tool-keyed bump rule assumes each ship bumps; nothing enforces it, and parallel sessions on the same day is exactly when it breaks.
- My v3.39.1 CHANGELOG subsection ended up wedged between the v3.39.0 heading and its body, because I inserted directly after the `## [3.39]` line while another session was restructuring the same region. Fixed here. Appending to a shared changelog mid-flight needs a re-read first, not a remembered line number.
- Found `main` with a stale `MERGE_HEAD` from the previous ship and `package.json` staged, which made the file unreadable to Node. The merge had already landed and pushed; only the marker was left. Harmless once diagnosed, but it means a ship left the working tree broken for the next session.
- Notion was three ships behind by the time it was reconciled (3.39.0, 3.39.1 and 3.39.2 were all unmirrored). Same drift the v3.38.2 entry flagged, so flagging it evidently is not enough.

**Next:**
- Consider a pre-push guard that fails when `package.json` version is unchanged but files outside `*.md` are. That converts the version-collision class of bug from "noticed three ships later" into "blocked at push".
- The long-tail heuristic is the only part of the 2026-06-20 blocklist item still open: a structural rule dropping outlets seen once with no corroboration, instead of hand-listing hyper-local regionals forever.
- `scripts/check-news-blocklist.mjs --self-test` exits non-zero, so it can gate a ship. Nothing calls it yet.

---

## 2026-07-25, v3.38.2, News blocklist weekly review batch 3

Applied the weekly source review: 44 WORLD and 46 ID outlets added to `BLOCKED_OUTLETS`. Proposal was generated by the scheduled task; application was a separate confirmed step, as designed.

**Well:**
- Verified by replaying the real match rule (`n === b || n.includes(b)`) over the week's actual pulled source names, rather than eyeballing the list. This caught two live traps that reading alone would have missed: the regional ANTARA bureaus need full phrases or a bare `antara` would kill the national wire, and `katadataoto` had to be checked so it did not swallow `databoks katadata`. A blocklist is exactly the kind of change where the diff looks obviously correct and is not.
- Refused two entries rather than shipping a silently wrong rule. `patch` would have matched "The Dispatch" and `komo` is a 4-letter token that collides too easily. Both are now noted in-file next to the pre-existing `ign` note, so the reason survives past this session.
- The proposal/apply split held. The scheduled task proposed and stopped; nothing was blocked until confirmed.

**Wrong:**
- The first verification script was itself buggy: the regex swallowed apostrophes inside comments (`// Women's Wear Daily`), which corrupted the parse and produced 41 false "not blocked" results. Nearly read that as a source-file problem. A checker that parses the file it is checking needs to be trusted less than the file.
- Shipped the blocklist as v3.38.2 before asking whether the full ship sequence was wanted, so docs and Notion lagged the runtime change by one step rather than landing with it.
- The scope of the instruction ("everything except Harapan Rakyat on Indonesia") was genuinely ambiguous between "the whole proposal" and "the ID list only". Proceeded on the broader reading with the assumption stated. Better would have been one clarifying question before touching 44 WORLD entries.

**Next:**
- Add a word-boundary matcher to `isBlockedOutlet`. Three outlets are now blocked on it (`patch`, `komo`, `ign`), and the count grows every review. On `docs/BACKLOG.md`.
- Consider committing the verification script rather than rebuilding it per review, so each weekly batch is checked the same way instead of by hand.
- README was still untouched `create-next-app` boilerplate, discovered during this ship's doc check. Rewritten here. Worth checking the repo's front door more than once a year.
...
**Well:**
- Ran a throwaway POC before writing any Jarvis code, and it paid for itself immediately. Four real bugs surfaced only by running it: Node `fetch` gets HTTP 200 with an empty body from Nitter while curl gets the real payload; CDATA-wrapped titles were being deleted outright by the tag stripper, silently zeroing four feeds; Reddit throttles on a rolling IP window and needs serialized fetches; and absolute star counts are not "trending". Every one of these fails silently as zero items rather than as an error, so none would have been caught by a green exit code.

  **Corrected in v3.38.1:** the empty-body finding was written up as "Nitter and Reddit", which was too broad. It is Nitter only. Reddit's failure is plain rate limiting that hits both transports; the original reading was a timing artifact of Reddit's rolling window.
