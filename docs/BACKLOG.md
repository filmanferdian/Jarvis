...
**Context:** v3.32.0 expanded `BLOCKED_OUTLETS` in `src/lib/sources/googleNewsRss.ts` by ~48 entries from a 14-day source audit, and stood up a weekly cloud routine (Sunday morning, over the previous 7 days) to keep proposing candidates for confirmation. Three things were intentionally deferred.
