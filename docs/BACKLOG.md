...
- Career budget/scheduling (surfaced in v3.19.3 when Grab/GoTo pushed kept to 145): scoring shares the 50/day Claude budget and is capped at 40/run, so a big catch-up run can eat most of the day's budget right before the 07:30 briefing. Either schedule the Tue/Thu career cron well clear of the briefing window, give career its own budget lane, or lower the per-run cap. Decide when wiring up the cron-job.org schedule.
