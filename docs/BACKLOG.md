...
- Scheduling (decided 2026-05-30): a cron-job.org job runs Monday and Wednesday at 19:00 WIB — evening, well clear of the 07:30 briefing. Cron `0 19 * * 1,3` in the Asia/Jakarta timezone, GET to the deployed `/api/cron/career-jobs` with the `x-cron-secret` header. This is a manual cron-job.org UI step; until it's created the pipeline only runs from the "Check now" button.
