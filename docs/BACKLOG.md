...
**Follow-ups:**
- **History backfill (only if Charge asks for it).** The ~102 existing runs have no detail row by design (no backfill). Each run needs up to 3 Garmin calls (`/splits` + `/weather` + `/details`), so ~300 calls vs a 50/day budget — it would have to be a throttled, resumable pass spread over several days, respecting the circuit breaker, processing runs missing from the table oldest-or-newest first. The fetch/build code already exists; only the throttled driver is missing. As of v3.35 this also covers the 12 coaching columns + `lap_detail` + per-run insights.
- **Retry incomplete rows.** A run whose `/splits` / `/weather` / `/details` failed once persists with null fields, and the sync then skips it because a row exists. Add a pass (or relax the "skip if row exists" check) to re-enrich rows where key fields are null, so transient endpoint failures self-heal on a later sync.
- **Dedicated Charge API token (v3.35).** `POST /api/running/insight` reuses `JARVIS_AUTH_TOKEN` (same as Ubayy). For independent revocation, add a `CHARGE_API_TOKEN` that `withAuth` also accepts, set it in Railway, and hand it to Charge instead of the shared token.
...
**Status (2026-07-26, v3.40.0): the long-tail heuristic is the only part still open.** The word-boundary matcher shipped as `WORD_BLOCKED_OUTLETS` in `src/lib/sources/googleNewsRss.ts`; `ign`, `patch` and `komo` are now blocked and their in-file NOTE comments are gone. The verification script shipped as `scripts/check-news-blocklist.mjs` with a 26-case self-test. The Tier-3 calls below were all decided in batch 4.

**Follow-ups:**
- **DONE in v3.39.1 — Word-boundary matcher.** Added `WORD_BLOCKED_OUTLETS`, matched with alphanumeric lookarounds (not `\b`, which treats `_` as a word character). `ign`, `patch` and `komo` moved into it. Future short tokens go there; ordinary multi-word outlet names stay in `BLOCKED_OUTLETS`.
- **DONE in v3.39.1 — Commit the verification script.** `scripts/check-news-blocklist.mjs` parses both lists out of the TypeScript source (no second copy), mirrors the match rule, and self-tests the collisions. Run `--self-test` before applying a weekly batch; pass outlet names to triage a proposal.
- **DONE in v3.40.0 — Tier-3 borderline decisions.** Mostly resolved in v3.38.2: `gizmodo` and `sfgate` were blocked; `san francisco chronicle`, `chicago sun-times` and `the texas tribune` were decided as keep; the Apple / gadget-rumor blogs were blocked back in batch 2. Batch 4 closed the rest: `detikinet` (13 appearances) and `cnet` (9) are **keep**, decided against blocking despite every sibling vertical being blocked, because both carry real tech news and together account for ~22 slots a week. `consumer reports`, `mashable`, `boing boing`, `seeking alpha` and `wisconsin examiner` were blocked. Finance content (stockbit snips, ajaib) and fact-checkers (turnbackhoax, jala hoaks) remain keep.
- **Long-tail heuristic.** Still open, and now the dominant cost. Batch 4 hand-listed 22 more one-off outlets (12 US call-sign affiliates and metro-print sites, 10 Indonesian regionals), every one at a single appearance for the week. Four batches have grown the lists to 321 entries (129 WORLD substring, 7 WORLD word-boundary, 185 ID) and the tail has not thinned. Consider a structural rule (drop an outlet seen once in a slot with no corroborating outlets) instead of growing the list forever.
- **Blocked by a matcher gap: bare aggregator labels.** `yahoo` appears as its own outlet name (2 appearances in the week to 2026-07-26) and cannot be blocked by either matcher: substring and word-boundary both also hit the legitimate "Yahoo Finance". Needs an exact-match tier (`n === b` only, no `includes`) to express "block the bare label, keep the verticals".
