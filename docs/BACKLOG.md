...
**Status (2026-07-25, v3.39.1): two of three DONE.** The word-boundary matcher shipped as `WORD_BLOCKED_OUTLETS` in `src/lib/sources/googleNewsRss.ts`; `ign`, `patch` and `komo` are now blocked and their in-file NOTE comments are gone. The verification script shipped as `scripts/check-news-blocklist.mjs` with a 26-case self-test. Verified with no regressions over the 185 real outlet names pulled in the week to 2026-07-25. Only the long-tail heuristic below remains open.
